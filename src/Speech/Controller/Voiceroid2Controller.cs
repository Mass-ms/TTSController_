...
namespace Speech
{
    ...
    public class Voiceroid2Controller : IDisposable, ISpeechController
    {
        ...
        {
            WPFTextBox textbox = new WPFTextBox(_root.IdentifyFromLogicalTreeIndex(0, 4, 5, 1, 1, 0, 3, 0, 6, (int)t, 0, 7));
        ...
        enum StyleType { Joy = 0, Anger = 1, Sadness = 2}

        public void SetJoy(float value)
        {
            SetStyle(StyleType.Joy, value);
        }
        public float GetJoy()
        {
            return GetStyle(StyleType.Joy);
        }
        public void SetAnger(float value)
        {
            SetStyle(StyleType.Anger, value);
        }
        public float GetAnger()
        {
            return GetStyle(StyleType.Anger);
        }
        public void SetSadness(float value)
        {
            SetStyle(StyleType.Sadness, value);
        }
        public float GetSadness()
        {
            return GetStyle(StyleType.Sadness);
        }
        private void SetStyle(StyleType t, float value)
        {
            WPFTextBox textbox = new WPFTextBox(_root.IdentifyFromVisualTreeIndex(0, 0, 0, 0, 1, 2, 0, 1, 0, 0, 0, 0, 0, 2, 0, 1, 0, 5, 0, 0, (int)t, 0, 0, 0, 0, 0, 3));
            textbox.EmulateChangeText($"{value:0.00}");
        }
        private float GetStyle(StyleType t)
        {
            WPFTextBox textbox = new WPFTextBox(_root.IdentifyFromVisualTreeIndex(0, 0, 0, 0, 1, 2, 0, 1, 0, 0, 0, 0, 0, 2, 0, 1, 0, 5, 0, 0, (int)t, 0, 0, 0, 0, 0, 3));
            return Convert.ToSingle(textbox.Text);
        }
